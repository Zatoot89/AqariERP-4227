...
export const seed = new Hono()
  .post("/", async (c) => {
    try {
      ...
      // ─── Demo users via Better Auth (scrypt hash) ─────────────────────────────
      const demoUsers = [
      ...
      for (const u of demoUsers) {
        // Use Better Auth's internal signup — this hashes with scrypt correctly
        const res = await auth.api.signUpEmail({
          body: { name: u.name, email: u.email, password: "demo1234" },
        });
      ...
      const [adminId, , agent1Id, agent2Id, agent3Id] = agentIds;
      ...
      for (const p of propData) {
        const pid = nanoid();
        propIds.push(pid);
        await db.insert(schema.properties).values({ id: pid, agencyId, listedBy: agent1Id, ...p });
      }
      ...
      const leadData = [
        ...
        { name: "Fatima Hassan", nameAr: "فاطمة حسن", phone: "+9647901234567", source: "whatsapp", stage: "new", propertyType: "apartment", budgetMin: 50000, budgetMax: 90000, currency: "USD", preferredArea: "Al-Mansour", assignedTo: agent1Id, whatsappId: "9647901234567", createdAt: now - 1 * day },
        ...
        { name: "Hassan Al-Sayed", nameAr: "حسن السيد", phone: "+9647781234567", source: "whatsapp", stage: "new", propertyType: "apartment", budgetMin: 60000, budgetMax: 100000, currency: "USD", preferredArea: "Erbil", assignedTo: agent1Id, whatsappId: "9647781234567", createdAt: now - 1 * day },
        ...
        { name: "Sophie Laurent", phone: "+971581234567", source: "propertyfinder", stage: "new", propertyType: "apartment", budgetMin: 1200000, budgetMax: 2000000, currency: "AED", preferredArea: "Downtown Dubai", assignedTo: agent3Id, createdAt: now - 1 * day },
      ];
      ...
      for (const l of leadData) {
        ...
        await db.insert(schema.leads).values({
          id: lid, agencyId,
          name: l.name, nameAr: l.nameAr ?? null,
          phone: l.phone, source: l.source, stage: l.stage,
          propertyType: l.propertyType, budgetMin: l.budgetMin, budgetMax: l.budgetMax,
          currency: l.currency, preferredArea: l.preferredArea,
          assignedTo: l.assignedTo, whatsappId: (l as any).whatsappId ?? null,
      ...
      for (let i = 0; i < Math.min(leadIds.length, 8); i++) {
        const count = (i % 3) + 2;
        for (const act of activityTemplates.slice(0, count)) {
          await db.insert(schema.activities).values({
            id: nanoid(), agencyId, leadId: leadIds[i],
            userId: agentIds[i % agentIds.length],
            type: act.type, body: act.body,
            meta: act.meta ? JSON.stringify(act.meta) : null,
      ...
      // ─── Lead-Property links ──────────────────────────────────────────────────
      await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[0], propertyId: propIds[0], status: "shown" });
      await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[1], propertyId: propIds[1], status: "interested" });
      await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[4], propertyId: propIds[2], status: "interested" });
      await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[7], propertyId: propIds[4], status: "shown" });

      // ─── Tasks ────────────────────────────────────────────────────────────────
      const taskData = [
        { title: "Call Ahmad about apartment viewing", titleAr: "اتصل بأحمد بشأن معاينة الشقة", type: "call", leadId: leadIds[0], assignedTo: agent1Id, dueAt: now + 1 * day, done: 0 },
        ...
        { title: "Confirm viewing with Ali — Erbil land", titleAr: "تأكيد معاينة علي — أرض أربيل", type: "viewing", leadId: leadIds[13], assignedTo: agent2Id, dueAt: now + 1 * day, done: 0 },
      ...
      for (const task of taskData) {
        await db.insert(schema.tasks).values({
          id: nanoid(), agencyId,
          title: task.title, titleAr: task.titleAr ?? null,
          type: task.type, leadId: task.leadId ?? null,
          assignedTo: task.assignedTo, createdBy: adminId,
          dueAt: task.dueAt, done: task.done,
        });
      }
      ...
      return c.json({
        ...
        summary: {
          ...
          properties: propData.length,
          tasks: taskData.length,
        },
        login: {
          email: "omar@demo.aqari",
          password: "demo1234",
        },
      }, 200);
    } catch (e: any) {
