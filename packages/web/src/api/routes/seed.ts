    ...
    for (let index = 0; index < Math.min(leadIds.length, 8); index += 1) {
      ...
      for (const activity of activityTemplates.slice(0, count)) {
        await db.insert(schema.activities).values({
          ...
          type: activity.type,
          body: activity.body,
          meta: activity.meta ? JSON.stringify(activity.meta) : null,
          createdAt: now - Math.floor(Math.random() * 7) * day,
        });
      }
    }

    await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[0], propertyId: propertyIds[0], status: "shown" });
    await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[1], propertyId: propertyIds[1], status: "interested" });
    await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[4], propertyId: propertyIds[2], status: "interested" });
    await db.insert(schema.leadProperties).values({ id: nanoid(), leadId: leadIds[7], propertyId: propertyIds[4], status: "shown" });
