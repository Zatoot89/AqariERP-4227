...
export const tasks = new Hono()
  .get("/", requireTenant, async (c) => {
    ...
    const { done, leadId, page, pageSize } = queryResult.data;
  ...
  .patch("/:id", requireTenant, async (c) => {
    ...
    const [task] = await db.update(schema.tasks).set({
      ...
      dueAt: body.dueAt,
      type: body.type,
    }).where(and(eq(schema.tasks.id, taskId), eq(schema.tasks.agencyId, agencyId))).returning();
  ...
  .patch("/:id/done", requireTenant, async (c) => {
    ...
    const [task] = await db.update(schema.tasks).set({ done: 1 })
      .where(and(eq(schema.tasks.id, taskId), eq(schema.tasks.agencyId, agencyId)))
      .returning();
    return c.json({ task }, 200);
  })
  .delete("/:id", requireTenant, async (c) => {
    ...
    await db.delete(schema.tasks).where(
      and(eq(schema.tasks.id, taskId), eq(schema.tasks.agencyId, agencyId)),
    );
