...
export const whatsapp = new Hono()
  .get("/leads/:id/messages", requireTenant, async (c) => {
    ...
    const messages = await db.select().from(schema.whatsappMessages)
      .where(and(
  ...
  .post("/leads/:id/send", requireTenant, async (c) => {
    const idResult = parseParam(c, entityIdSchema);
    if (!idResult.success) return idResult.response;
    const bodyResult = await parseJson(c, sendWhatsappMessageSchema);
    ...
    const agencyId = c.get("agencyId") as string;
    const leadId = idResult.data;
    const text = bodyResult.data.body;
  ...
  .post(
    ...
    async (c) => {
      ...
      await db.update(schema.agencies).set({ waConnectedAt: Date.now() })
