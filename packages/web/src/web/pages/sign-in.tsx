...
export default function SignInPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (err) { setLoading(false); setError(err.message ?? "Sign up failed"); return; }
    // Auto-create profile after signup
    try {
      await fetch("/api/settings/agency", { headers: { Authorization: `Bearer ${localStorage.getItem("aqari_token")}` } });
    } catch {}
    setLoading(false);
    setLocation("/dashboard");
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: "var(--bg)" }}>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>{t("app_name")}</p>
        </div>
        ...
        <div className="card p-6">
          <form onSubmit={tab === "signin" ? handleSignIn : handleSignUp} className="space-y-4">
            ...
            <button type="submit" className="btn-primary w-full py-2.5" disabled={loading}>
              {loading
                ? (tab === "signin" ? t("auth.signing_in") : t("auth.signing_up"))
                : (tab === "signin" ? t("auth.sign_in") : t("auth.sign_up"))}
            </button>
          </form>
        </div>
